...
**Decision**: 所有部署变量通过 `?=` 设置 QA 环境默认值；`deploy-qa` **无需传参即可运行**；`DEPLOY_USR` 新增，默认值 `ubuntu`
...
deploy-qa: build-linux
	...
	@ssh $(DEPLOY_USR)@$(DEPLOY_HOST) "supervisorctl restart $(DEPLOY_SUPERVISOR)"
```

**Alternatives considered**:
- 保持变量空默认值 + `runqa` 目标特定变量覆盖：rejected，变量定义分散在多处，不够直观；且 `deploy-qa` 仍需手动传参
- 每个部署目标（QA/Prod）各设一套默认值：rejected，本项目的 QA 环境是唯一稳定部署目标，过度设计

---

## R2: `runqa` 目标实现方式

**Decision**: `runqa` 直接依赖 `deploy-qa`，无需目标特定变量或额外逻辑
...
**Alternatives considered**:
- 使用目标特定变量预设 QA 参数：rejected，变量默认值已包含 QA 配置，冗余且容易造成默认值不一致
- 创建独立 `deploy-runqa` 目标：rejected，代码重复，背离 DRY 原则
- 使用 `export VAR=... && $(MAKE) deploy-qa` 子 make：rejected，额外进程开销

---

## R3: 变量覆盖优先级验证

**Decision**: 依赖 GNU Make 标准变量优先级机制，所有变量均可通过命令行覆盖
...
**Decision**: `runqa` 目标使用与现有目标一致的 `## deploy:` 注释格式，中英双语描述
...
**Decision**: 更新用法提示，列出所有可用参数（含 `DEPLOY_USR`）
